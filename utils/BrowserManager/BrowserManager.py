...
class BrowserManager():
    ...
    async def get_new_page(self, mobile_mode=False) -> Page:
        ...
        if mobile_mode:
            ...
            await page.setViewport(viewport={'width': 414, 'height': 736})
        else:
            await page.setUserAgent(
                'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Safari/537.36')
            await page.setViewport(viewport={'width': 1536, 'height': 768, 'isMobile': True, 'hasTouch': True})
    ...
    async def refresh_browser(self) -> str:
        if await self.get_page_count() > 1:
            return "尚有任务正在执行"
